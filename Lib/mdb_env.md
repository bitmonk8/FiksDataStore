...
Defined in [`Lib/mdb_env.h:120`](Lib/mdb_env.h:120), this is the opaque handle that represents the entire database environment. It holds all state associated with an open database.
...
*   `me_flags`: A bitmask of flags that define the environment's behavior (e.g., `MDB_RDONLY`, `MDB_WRITEMAP`, `MDB_NOSYNC`).
...
*   `me_free_pgs`, `me_dirty_list`: Data structures (`MDB_IDL`, `MDB_ID2L`) for managing free and dirty pages within a write transaction.
...
Defined in [`Lib/mdb_env.h:18`](Lib/mdb_env.h:18), this structure represents the header of a database snapshot. Two instances of this structure exist at the beginning of the data file, on page 0 and page 1.
...
*   `mm_dbs[CORE_DBS]`: An array of two `MDB_db` structures.
...
### 2.3. `struct MDB_txninfo` & `struct MDB_txbody`

Defined in [`Lib/mdb_env.h:86`](Lib/mdb_env.h:86), these structures define the layout of the lock file (`lock.mdb`). The lock file is also memory-mapped and is crucial for coordinating concurrent access.
...
*   `mti_readers[]`: A flexible array of `MDB_reader` structures. This is the **Reader Table**. Each active read transaction acquires a slot in this table and records its process ID and the `txnid` of the snapshot it is reading. This table is what allows the writer to determine which old pages are no longer in use and can be reclaimed.

---

## 3. Main Algorithms and Operations

### 3.1. Environment Opening (`mdb_env_open`)

This is the primary function for initializing the database environment. It is a multi-step process orchestrated by [`mdb_env_open()`](Lib/mdb_env.cpp:1566).

1.  **Path and Flag Handling:** It resolves the path to the database directory and validates the user-provided flags.
2.  **Lock File Setup (`mdb_env_setup_locks`)**:
...
3.  **Data File Setup (`mdb_env_open2`)**:
    *   It opens (or creates) the data file (`data.mdb`).
    *   **Header Reading (`mdb_env_read_header`)**: It reads both meta pages (0 and 1) to determine which is newer by comparing their `mm_txnid` values.
    *   **Initialization**: If the data file is new, it calls `mdb_env_init_meta` to create the initial two meta pages.
    *   **Memory Mapping (`mdb_env_map`)**: It memory-maps the data file using `mmap` (POSIX) or `NtMapViewOfSection` (Windows). The `MDB_WRITEMAP` flag determines if the mapping is read-write or read-only.
    *   It sets up internal environment parameters (`me_maxpg`, `me_nodemax`, etc.) based on the page size.

### 3.2. Transactional Commit (`mdb_env_write_meta`)

This function, defined in [`Lib/mdb_env.cpp:631`](Lib/mdb_env.cpp:631), is the final step of a successful write transaction.

1.  **Toggle Meta Page:** It determines which meta page to write to based on the new transaction ID (`txn->mt_txnid & 1`).
2.  **Construct New Meta:** It builds a new `MDB_meta` structure containing the updated state from the committed transaction, including the new root page of the main B-tree, the new root of the free-pages B-tree, and the new last-allocated page number.
...
### 3.4. Environment Copy (`mdb_env_copyfd2`)

The copy functionality, defined in [`Lib/mdb_env.cpp:2426`](Lib/mdb_env.cpp:2426), provides two modes:

1.  **As-is Copy (`mdb_env_copyfd0`)**: This performs a direct, byte-for-byte copy of the active pages in the database file. It takes a read lock to ensure it's copying a consistent state.
2.  **Compacting Copy (`mdb_env_copyfd1`)**: This is a more sophisticated operation designed to reduce file size by removing fragmentation.
    *   It launches a dedicated writer thread (`mdb_env_copythr`) to handle file I/O.
    *   The main thread performs a depth-first traversal of the B-tree (`mdb_env_cwalk`).
...
*   **POSIX (`MDB_USE_POSIX_MUTEX`)**: The preferred modern Unix implementation. Uses `mmap` and process-shared, robust `pthread_mutex_t` mutexes stored directly in the lock file's shared memory.
*   **POSIX Semaphores (`MDB_USE_POSIX_SEM`)**: A fallback for systems (like macOS) that don't support process-shared mutexes. It uses named POSIX semaphores (`sem_open`).
*   **System V Semaphores (`MDB_USE_SYSV_SEM`)**: A fallback for older Unix systems, using `semget` and `semctl`.
