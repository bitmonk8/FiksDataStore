...
*   `fds_strdup`: A platform-independent macro for string duplication, aliasing to `_strdup` on Windows and `strdup` on other systems.

### Constants

*   `FDS_COMMIT_PAGES`: Defines the maximum number of pages to commit in a single `writev()` system call. This value is capped by `IOV_MAX` if it is defined and smaller.
