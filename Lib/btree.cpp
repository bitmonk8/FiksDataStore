...
static auto fds_page_loose(FDS_cursor* mc, FDS_page* mp) -> int
{
    ...
    if (((mp->mp_flags & P_DIRTY) != 0) && mc->mc_dbi != FREE_DBI)
    {
        if (txn->mt_parent != nullptr)
        {
            ...
            if (dl[0].mid != 0U)
            {
                ...
                const auto x = fds_mid2l_search(dl, pgno);
    ...
    else
    {
        ...
        const auto rc = fds_midl_append(&txn->mt_free_pgs, pgno);
        if (rc != 0)
            return rc;
    }
...
auto fds_page_touch(FDS_cursor* mc) -> int
{
    ...
    {
        if ((txn->mt_flags & FDS_TXN_SPILLS) != 0U)
        {
            FDS_page* unspilled_page = nullptr;
            if (const auto rc = fds_page_unspill(txn, mp, &unspilled_page); rc != 0)
            {
                txn->mt_flags |= FDS_TXN_ERROR;
                return rc;
            }
        ...
        if (const auto rc = fds_midl_need(&txn->mt_free_pgs, 1); rc != 0)
        {
            txn->mt_flags |= FDS_TXN_ERROR;
            return rc;
        }
        if (const auto rc = fds_page_alloc(mc, 1, &new_page); rc != 0)
        {
            txn->mt_flags |= FDS_TXN_ERROR;
            return rc;
        }
        ...
        fds_cassert(mc, mp->mp_pgno != new_pgno);
        ...
        fds_page_copy(new_page, mp, txn->mt_env->me_psize);
    ...
    else if ((txn->mt_parent != nullptr) && !IS_SUBP(mp))
    {
        ...
        if (dl[0].mid != 0U)
        {
            const auto x = fds_mid2l_search(dl, current_pgno);
        ...
        fds_cassert(mc, dl[0].mid < FDS_IDL_UM_MAX);
        // No - copy it
        new_page = fds_page_malloc(txn, 1);
        if (new_page == nullptr)
            return ENOMEM;
        FDS_ID2 mid;
        mid.mid = current_pgno;
        mid.mptr = new_page;
        const auto rc = fds_mid2l_insert(dl, &mid);
        fds_cassert(mc, rc == 0);
        fds_page_copy(new_page, mp, txn->mt_env->me_psize);
...
auto fds_page_search_root(FDS_cursor* mc, FDS_val* key, int modify) -> int
{
    ...
    while (IS_BRANCH(mp))
    {
        ...
        fds_cassert(mc, !mc->mc_dbi || NUMKEYS(mp) > 1);
        DPRINTF(("found index 0 to page %" Yu, NODEPGNO(NODEPTR(mp, 0))));

        indx_t i;
        bool descend = true;

        if ((modify & (FDS_PS_FIRST | FDS_PS_LAST)) != 0)
        {
            i = 0;
            if ((modify & FDS_PS_LAST) != 0)
            {
                i = NUMKEYS(mp) - 1;
        ...
        {
            int exact;
            auto* search_node = fds_node_search(mc, key, &exact);
        ...
        if (descend)
        {
            fds_cassert(mc, i < NUMKEYS(mp));
            const auto* node = NODEPTR(mp, i);

            FDS_page* child_page;
            if (const auto rc = fds_page_get(mc, NODEPGNO(node), &child_page, nullptr); rc != 0)
            {
                return rc;
            }
            mp = child_page;
            ...
            if (const auto rc = fds_cursor_push(mc, mp); rc != 0)
            {
                return rc;
            }
        }

        if ((modify & FDS_PS_MODIFY) != 0)
        {
            if (const auto rc = fds_page_touch(mc); rc != 0)
            {
                return rc;
            }
            mp = mc->mc_pg[mc->mc_top];
...
auto fds_page_search_lowest(FDS_cursor* mc) -> int
{
    ...
    FDS_page* new_page = nullptr;
    if (const auto rc = fds_page_get(mc, NODEPGNO(node), &new_page, nullptr); rc != 0)
        return rc;

    mc->mc_ki[mc->mc_top] = 0;

    if (const auto rc = fds_cursor_push(mc, new_page); rc != 0)
        return rc;

    return fds_page_search_root(mc, nullptr, FDS_PS_FIRST);
}
...
auto fds_page_search(FDS_cursor* mc, FDS_val* key, int flags) -> int
{
    ...
    if ((*mc->mc_dbflag & DB_STALE) != 0)
    {
        ...
        int exact = 0;
        const auto leaf = fds_node_search(&mc2, &mc->mc_dbx->md_name, &exact);
        if (exact == 0)
            return FDS_BAD_DBI;

        if ((leaf->mn_flags & (F_DUPDATA | F_SUBDATA)) != F_SUBDATA)
            return FDS_INCOMPATIBLE;  // not a named DB

        FDS_val data;
        rc = fds_node_read(&mc2, leaf, &data);
        if (rc != 0)
            return rc;

        uint16_t db_flags;
        memcpy(&db_flags, (char*)data.mv_data + offsetof(FDS_db, md_flags), sizeof(uint16_t));
        // The txn may not know this DBI, or another process may
        // have dropped and recreated the DB with other flags.
        if ((mc->mc_db->md_flags & PERSISTENT_FLAGS) != db_flags)
            return FDS_INCOMPATIBLE;

        memcpy(mc->mc_db, data.mv_data, sizeof(FDS_db));
    ...
    fds_cassert(mc, root > 1);
    if ((mc->mc_pg[0] == nullptr) || mc->mc_pg[0]->mp_pgno != root)
    {
        rc = fds_page_get(mc, root, &mc->mc_pg[0], nullptr);
        if (rc != 0)
            return rc;
    }
    ...
    if ((flags & FDS_PS_ROOTONLY) != 0)
        return FDS_SUCCESS;

    return fds_page_search_root(mc, key, flags);
}
...
auto fds_node_add(FDS_cursor* mc, indx_t indx, FDS_val* key, FDS_val* data, pgno_t pgno, unsigned int flags) -> int
{
    ...
    FDS_node* node;
    FDS_page* mp = mc->mc_pg[mc->mc_top];
    FDS_page* ofp = nullptr;  // overflow page
    void* ndata;
    DKBUF;

    fds_cassert(mc, MP_UPPER(mp) >= MP_LOWER(mp));
    ...
    if (IS_LEAF(mp))
    {
        ...
        else if (node_size + data->mv_size > mc->mc_txn->mt_env->me_nodemax)
        {
            ...
            if (node_size > room)
                goto full;
            rc = fds_page_new(mc, P_OVERFLOW, ovpages, &ofp);
            if (rc != 0)
                return rc;
...
update:
    ...
    fds_cassert(mc, ofs >= MP_LOWER(mp) + sizeof(indx_t));
    MP_PTRS(mp)[indx] = ofs;
    MP_UPPER(mp) = ofs;
    ...
    if (IS_LEAF(mp))
    {
        ...
        if (ofp == nullptr)
        {
            ...
            else if (F_ISSET(flags, FDS_RESERVE))
                data->mv_data = ndata;
            else
...
full:
#if FDS_DEBUG
    DPRINTF(("not enough room in page %" Yu ", got %u ptrs", fds_dbg_pgno(mp), NUMKEYS(mp)));
#endif
    DPRINTF(("upper-lower = %u - %u = %" Z "d", MP_UPPER(mp), MP_LOWER(mp), room));
    DPRINTF(("node size = %" Z "u", node_size));
    mc->mc_txn->mt_flags |= FDS_TXN_ERROR;
    return FDS_PAGE_FULL;
}
...
#if FDS_DEBUG
    DPRINTF(("delete node %u on %s page %" Yu, indx, IS_LEAF(mp) ? "leaf" : "branch", fds_dbg_pgno(mp)));
#endif
    numkeys = NUMKEYS(mp);
    fds_cassert(mc, indx < numkeys);
...
void fds_node_shrink(FDS_page* mp, indx_t indx)
{
    ...
    node = NODEPTR(mp, indx);
    sp = reinterpret_cast<FDS_page*>(reinterpret_cast<char*>(node->mn_data) + node->mn_ksize);
    delta = SIZELEFT(sp);
    nsize = NODEDSZ(node) - delta;

    /* Prepare to shift upward, set len = length(subpage part to shift) */
    xp = reinterpret_cast<FDS_page*>(reinterpret_cast<char*>(sp) + delta); /* destination subpage */
...
auto fds_node_move(FDS_cursor* csrc, FDS_cursor* cdst, int fromleft) -> int
{
    ...
    rc = fds_page_touch(csrc);
    if (rc != 0)
        return rc;
    rc = fds_page_touch(cdst);
    if (rc != 0)
        return rc;

    {
        srcnode = NODEPTR(csrc->mc_pg[csrc->mc_top], csrc->mc_ki[csrc->mc_top]);
        fds_cassert(csrc, !((size_t)srcnode & 1));
        ...
        if (csrc->mc_ki[csrc->mc_top] == 0 && IS_BRANCH(csrc->mc_pg[csrc->mc_top]))
        {
            ...
            rc = fds_page_search_lowest(csrc);
            if (rc != 0)
                return rc;
     ...
     */
    rc = fds_node_add(cdst, cdst->mc_ki[cdst->mc_top], &key, &data, srcpg, flags);
    if (rc != FDS_SUCCESS)
        return rc;

    /* Delete the node from the source page.
     */
    fds_node_del(csrc, key.mv_size);

    {
        /* Adjust other cursors pointing to mp */
        FDS_cursor* m2;
        FDS_cursor* m3;
        FDS_dbi dbi = csrc->mc_dbi;
        FDS_page* mpd;
        FDS_page* mps;
    ...
    if (csrc->mc_ki[csrc->mc_top] == 0)
    {
        if (csrc->mc_ki[csrc->mc_top - 1] != 0)
        {
            ...
            mn.mc_snum--;
            mn.mc_top--;
            /* We want fds_rebalance to find mn when doing fixups */
            // fds_update_key is static in fds_cursor.cpp
            WITH_CURSOR_TRACKING(mn, rc = fds_update_key(&mn, &key));
            if (rc != 0)
                return rc;
        }
        if (IS_BRANCH(csrc->mc_pg[csrc->mc_top]))
        {
            ...
            rc = fds_update_key(csrc, &nullkey);
            csrc->mc_ki[csrc->mc_top] = ix;
            fds_cassert(csrc, rc == FDS_SUCCESS);
        }
    }
    ...
    if (cdst->mc_ki[cdst->mc_top] == 0)
    {
        ...
        if (IS_BRANCH(cdst->mc_pg[cdst->mc_top]))
        {
            ...
            // fds_update_key is static in fds_cursor.cpp
            rc = fds_update_key(cdst, &nullkey);
            cdst->mc_ki[cdst->mc_top] = ix;
            fds_cassert(cdst, rc == FDS_SUCCESS);
        }
    }
...
{
    FDS_page* psrc{nullptr};
    FDS_page* pdst{nullptr};
    FDS_node* srcnode{nullptr};
    FDS_val key{};
    FDS_val data{};
    unsigned nkeys{};
    ...
    fds_cassert(csrc, csrc->mc_snum > 1);  // can't merge root page
    fds_cassert(csrc, cdst->mc_snum > 1);
    ...
    for (unsigned int i = 0; i < NUMKEYS(psrc); i++, j++)
    {
        ...
        data.mv_size = NODEDSZ(srcnode);
        data.mv_data = NODEDATA(srcnode);
        rc = fds_node_add(cdst, j, &key, &data, NODEPGNO(srcnode), srcnode->mn_flags);
        if (rc != FDS_SUCCESS)
            return rc;
    }
     ...
     */
    // fds_page_loose is static in this file
    rc = fds_page_loose(csrc, psrc);
    if (rc != 0)
        return rc;
    if (IS_LEAF(psrc))
...
auto fds_rebalance(FDS_cursor* mc) -> int
{
    ...
    DPRINTF(("rebalancing %s page %" Yu " (has %u keys, %.1f%% full)",
             ...
             fds_dbg_pgno(mc->mc_pg[mc->mc_top]),
    ...
    {
#if FDS_DEBUG
        DPRINTF(("no need to rebalance page %" Yu ", above fill threshold", fds_dbg_pgno(mc->mc_pg[mc->mc_top])));
#endif
        return FDS_SUCCESS;
    }
    ...
    if (mc->mc_snum < 2)
    {
        ...
        if (NUMKEYS(mp) == 0)
        {
            ...
            // fds_midl_append is from midl.h
            rc = fds_midl_append(&mc->mc_txn->mt_free_pgs, mp->mp_pgno);
            if (rc != 0)
                return rc;
            /* Adjust cursors pointing to mp */
            mc->mc_snum = 0;
        ...
        else if (IS_BRANCH(mp) && NUMKEYS(mp) == 1)
        {
            ...
            // fds_page_get is in fds_page_io.h
            rc = fds_page_get(mc, mc->mc_db->md_root, &mc->mc_pg[0], nullptr);
            if (rc != 0)
                return rc;
            mc->mc_db->md_depth--;
            mc->mc_db->md_branch_pages--;
     ...
     */
    ptop = mc->mc_top - 1;
    fds_cassert(mc, NUMKEYS(mc->mc_pg[ptop]) > 1);
    ...
    if (mc->mc_ki[ptop] == 0)
    {
        ...
        // fds_page_get is in fds_page_io.h
        rc = fds_page_get(mc, NODEPGNO(node), &mn.mc_pg[mn.mc_top], nullptr);
        if (rc != 0)
            return rc;
    ...
    {
        rc = fds_node_move(&mn, mc, fromleft);
        if (fromleft != 0)
        {
    ...
    else
    {
        ...
        else
        {
            ...
            // We want fds_rebalance to find mn when doing fixups
            WITH_CURSOR_TRACKING(mn, rc = fds_page_merge(mc, &mn));
            fds_cursor_copy(&mn, mc);
        }
        mc->mc_flags &= ~C_EOF;
    }
...
auto fds_page_split(FDS_cursor* mc, FDS_val* newkey, FDS_val* newdata, pgno_t newpgno, unsigned int nflags) -> int
{
    ...
    FDS_page* rp{nullptr};
    const int pageNewResult = fds_page_new(mc, mp->mp_flags, 1, &rp);
    if (pageNewResult != 0)
    ...
    // height is 1. But when called from fds_update_key,
    // the cursor height may be greater because it walks
    // up the stack while finding the branch slot to update.
    if (mc->mc_top < 1)
    {
        FDS_page* pp{nullptr};
        const int pageNewResult = fds_page_new(mc, P_BRANCH, 1, &pp);
        if (pageNewResult != 0)
        {
            mc->mc_txn->mt_flags |= FDS_TXN_ERROR;
            return pageNewResult;
        ...
        const int nodeAddResult = fds_node_add(mc, 0, nullptr, nullptr, mp->mp_pgno, 0);
        if (nodeAddResult != FDS_SUCCESS)
        {
            // undo the pre-push
    ...
    FDS_page* copy{nullptr};
    int split_indx{};
    FDS_val sepkey{};
    FDS_node* node{nullptr};
    FDS_env* env = mc->mc_txn->mt_env;

    if ((nflags & FDS_APPEND) != 0U)
    {
        mn.mc_ki[mn.mc_top] = 0;
        sepkey = *newkey;
        split_indx = newindx;
    ...
    else
    {
...

        {
            ...
            const int nsize = EVEN(IS_LEAF(mp) ? fds_leaf_size(env, newkey, newdata) : fds_branch_size(env, newkey));

            // grab a page to hold a temporary copy
            copy = fds_page_malloc(mc->mc_txn, 1);
            if (copy == nullptr)
            {
                if (copy != nullptr)  // tmp page
                    fds_page_free(env, copy);
                mc->mc_txn->mt_flags |= FDS_TXN_ERROR;
                return ENOMEM;
            }
            ...
            // difference between success or failure in fds_node_add.
            ...
            if (nkeys < keythresh || nsize > pmax / 16 || newindx >= nkeys)
            {
                ...
                for (; i != k; i += j)
                {
                    ...
                    {
                        node = reinterpret_cast<FDS_node*>(reinterpret_cast<char*>(mp) + copy->mp_ptrs[i] + PAGEBASE);
            ...
            {
                node = reinterpret_cast<FDS_node*>(reinterpret_cast<char*>(mp) + copy->mp_ptrs[split_indx] + PAGEBASE);
    ...
    if (SIZELEFT(mn.mc_pg[ptop]) < fds_branch_size(env, &sepkey))
    {
        int snum = mc->mc_snum;
        mn.mc_snum--;
        mn.mc_top--;
        ...
        int pageSplitResult = FDS_SUCCESS;
        WITH_CURSOR_TRACKING(mn, pageSplitResult = fds_page_split(&mn, &sepkey, nullptr, rp->mp_pgno, 0));
        ...
        if (mn.mc_pg[ptop] != mc->mc_pg[ptop] && mc->mc_ki[ptop] >= NUMKEYS(mc->mc_pg[ptop]))
        {
            ...
            else
            {
                ...
                int cursorSiblingResult = fds_cursor_sibling(mc, 0);
                if (cursorSiblingResult != FDS_SUCCESS)
                {
                    if (cursorSiblingResult == FDS_NOTFOUND)  // improper fds_cursor_sibling() result
                        cursorSiblingResult = FDS_PROBLEM;
                    if (copy != nullptr)  // tmp page
                        fds_page_free(env, copy);
                    mc->mc_txn->mt_flags |= FDS_TXN_ERROR;
    ...
    {
        mn.mc_top--;
        int nodeAddResult = fds_node_add(&mn, mn.mc_ki[ptop], &sepkey, nullptr, rp->mp_pgno, 0);
        mn.mc_top++;
        if (nodeAddResult != FDS_SUCCESS)
        {
            if (nodeAddResult == FDS_NOTFOUND)  // improper fds_cursor_sibling() result
                nodeAddResult = FDS_PROBLEM;
            if (copy != nullptr)  // tmp page
                fds_page_free(env, copy);
            mc->mc_txn->mt_flags |= FDS_TXN_ERROR;
            return nodeAddResult;
    ...
    if ((nflags & FDS_APPEND) != 0U)
    {
        ...
        mc->mc_ki[mc->mc_top] = 0;
        const int nodeAddResult = fds_node_add(mc, 0, newkey, newdata, newpgno, nflags);
        if (nodeAddResult != 0)
        {
            if (copy != nullptr)  // tmp page
                fds_page_free(env, copy);
    ...
    else
    {
        ...
        do
        {
            ...
            const int nodeAddResult = fds_node_add(mc, j, &rkey, rdata, pgno, flags);
            if (nodeAddResult != 0)
            {
                if (copy != nullptr)  // tmp page
                    fds_page_free(env, copy);
...

    {
        ...
        for (m2 = mc->mc_txn->mt_cursors[dbi]; m2 != nullptr; m2 = m2->mc_next)
        {
            ...
            {
                if (m3->mc_ki[mc->mc_top] >= newindx && ((nflags & FDS_SPLIT_REPLACE) == 0U))
                    m3->mc_ki[mc->mc_top]++;
