...
The core design revolves around a special "main database" that acts as a directory, storing the metadata for all other "named databases" within the environment. Operations are transaction-based, ensuring ACID properties. The implementation uses integer handles ([`MDB_dbi`](Lib/db.h:29)) rather than pointers to refer to databases, which enhances safety and simplifies management across multiple transactions.

## 2. Core Data Structures

Two primary structures define a database's state: one for persistent on-disk metadata ([`MDB_db`](Lib/db.h:6)) and one for transient in-memory data ([`MDB_dbx`](Lib/db.h:21)).
...
-   [`uint16_t md_flags`](Lib/db.h:9): Persistent flags for the database, such as `MDB_REVERSEKEY`. These are set when the database is opened via [`dbi_open()`](Lib/db.cpp:22).
...
-   [`mdb_size_t md_entries`](Lib/db.h:14): The total number of key-value pairs stored in the database.
...
-   [`MDB_val md_name`](Lib/db.h:23): The name of the database.
-   [`MDB_cmp_func md_cmp`](Lib/db.h:24): A function pointer to the key-comparison function. This is determined by the `md_flags` (e.g., `MDB_REVERSEKEY`) but can be overridden by the user with [`set_compare()`](Lib/db.cpp:430).
-   [`MDB_cmp_func md_dcmp`](Lib/db.h:25): A function pointer for comparing data items in databases with sorted duplicates. In this version of the code, this is unused as duplicate support has been simplified out.
...
The environment contains a special "main database" accessible via the handle `MAIN_DBI`. This database acts as a directory or catalog, mapping database names (as keys) to their corresponding [`MDB_db`](Lib/db.h:6) metadata structures (as values). When a named database is opened, its metadata is read from this main database.

### 3.3. DBI Handle Lifecycle and Validation

-   **Opening**: A handle is obtained via [`mdb_dbi_open()`](Lib/db.cpp:22). If the database is already open in the current transaction, the existing handle is returned. Otherwise, the function searches the main DB for the database's metadata. If not found and `MDB_CREATE` is specified, a new database is created.
...
6.  **If found**: The associated value is the database's [`MDB_db`](Lib/db.h:6) metadata.
7.  **If not found**:
    -   If the `MDB_CREATE` flag was not provided, return `MDB_NOTFOUND`.
    -   If `MDB_CREATE` was provided, create a new, empty [`MDB_db`](Lib/db.h:6) record and write it to the main database, using the `name` as the key.
...
9.  Set the default comparison function using [`mdb_default_cmp()`](Lib/db.cpp:13).

### 4.2. `mdb_drop0()`

[`mdb_drop0()`](Lib/db.cpp:181) is the internal function to perform the core work of freeing a database's pages.
...
-   [`mdb_get(txn, dbi, key, data)`](Lib/db.cpp:439): Initializes a cursor, seeks to the given `key` using `MDB_SET`, and returns the found `data`.
-   [`mdb_put(txn, dbi, key, data, flags)`](Lib/db.cpp:399): Initializes a cursor and calls [`mdb_cursor_put_impl()`](Lib/cursor.cpp) to perform the write operation, handling flags like `MDB_NOOVERWRITE` and `MDB_APPEND`.
-   [`mdb_del(txn, dbi, key, data)`](Lib/db.cpp:320): Initializes a cursor, seeks to the item, and calls [`mdb_cursor_del_impl()`](Lib/cursor.cpp) to perform the deletion.
