# MDB Debugging Utilities (`mdb_debug.h`, `mdb_debug.cpp`)

## 1. Purpose

The `mdb_debug.h` header and its corresponding implementation `mdb_debug.cpp` provide a suite of debugging tools for the MDB library. These utilities are designed for developers working with or on MDB to diagnose issues, inspect the internal state of the database, and validate its integrity.

The entire debugging framework is designed to be conditionally compiled. By default, it is disabled, ensuring that production builds do not carry any of the performance overhead associated with debugging checks and logging. Activation is controlled via preprocessor macros, primarily `MDB_DEBUG` and the standard `NDEBUG`.
...
*   **Macros:**
    *   `mdb_cassert(mc, expr)`: Asserts a condition within a cursor context (`MDB_cursor`).
    *   `mdb_tassert(txn, expr)`: Asserts a condition within a transaction context (`MDB_txn`).
*   **Failure Handling:**
    *   When an assertion fails, the `mdb_assert_fail()` function is invoked.
    *   It constructs a detailed error message including the failed expression, function name, file, and line number.
    *   If a custom assertion handler has been set on the environment (`MDB_env.me_assert_func`), it is called with the message.
...
*   **Key Formatting:**
    *   `DKEY(mdb_val)`: A macro that converts a key (`MDB_val`) into a hexadecimal string for printing.
    *   It uses the underlying function `mdb_dkey()`, which handles the conversion.
    *   This requires a buffer named `kbuf` to be in scope, which is provided by the `DKBUF` macro.
*   **Page and Node Inspection:**
    *   `mdb_page_list(MDB_page *mp)`: Prints a comprehensive summary of a single database page. It identifies the page type (Branch, Leaf, Overflow, Meta), its flags (e.g., `P_DIRTY`), and lists all nodes on the page with their keys and sizes.
    *   `mdb_leafnode_type(MDB_node *n)`: Returns a descriptive string for a leaf node's type (e.g., if it contains sub-database data, overflow data, etc.).
...
*   **Function:** `mdb_audit(MDB_txn *txn)`
*   **Activation:** This function is only available when `MDB_DEBUG` is set to a value greater than 2.
*   **Algorithm:**
...
*   `MDB_env`: The environment, which can hold a pointer to a custom assertion handler.
*   `MDB_txn`: The transaction, which provides the context for auditing and assertions.
*   `MDB_page`: A database page, the primary subject of `mdb_page_list`.
*   `MDB_node`: A node within a page, representing a key/value entry.
*   `MDB_val`: The standard structure for representing keys and data.

## 4. How to Use

To leverage these debugging tools, a developer would typically perform the following steps:

1.  **Enable Debugging:** In the build configuration or before including MDB headers, define the `MDB_DEBUG` macro to the desired level (1, 2, or 3).
