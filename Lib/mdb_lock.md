...
Defined in [`Lib/mdb_lock.h:43`](Lib/mdb_lock.h:43), this is the fundamental structure for tracking a single read transaction. An array of these structures forms the "reader table" in shared memory.
...
struct MDB_reader {
    union {
        struct MDB_rxbody {
            ...
            volatile MDB_PID_T mrb_pid; // The process ID of the reader
            volatile MDB_THR_T mrb_tid; // The thread ID of the reader
        } mrx;
        // Cache-line padding
        char pad[...];
    } mru;
...
The entire `MDB_reader` struct is padded to a full cache line to prevent "false sharing" performance issues when multiple cores are concurrently accessing different reader slots.

## 3. Core Algorithms

### 3.1. Stale Reader Detection and Cleanup

The main function for this is [`mdb_reader_check0()`](Lib/mdb_lock.cpp:213). It is called to find and purge reader table entries left behind by dead processes.

**Algorithm:**

1.  **Collect Unique PIDs**: The function iterates through the entire reader table. For every active reader slot (`mr_pid != 0`) belonging to a process other than the current one, it adds the process ID (`PID`) to a temporary list. A helper function, [`mdb_pid_insert()`](Lib/mdb_lock.cpp:108), uses a binary search to efficiently build a sorted list of unique PIDs.
2.  **Check Liveness**: It then iterates through this list of unique PIDs. For each PID, it calls [`mdb_reader_pid()`](Lib/mdb_lock.cpp:23) to determine if the process is still alive.
3.  **Clear Stale Entries**: If [`mdb_reader_pid()`](Lib/mdb_lock.cpp:23) reports that a process is dead, the function must clear its entries from the reader table.
...
### 3.2. Process Liveness Check (`mdb_reader_pid`)

This function ([`Lib/mdb_lock.cpp:23`](Lib/mdb_lock.cpp:23)) provides a platform-specific way to check if a process is running.
...
### 3.3. Mutex Failure Recovery (`mdb_mutex_failed`)

This function ([`Lib/mdb_lock.cpp:164`](Lib/mdb_lock.cpp:164)) handles the critical scenario where a process dies while holding a mutex.

-   When a mutex lock attempt returns `MDB_OWNERDEAD` (or a platform-specific equivalent like `EOWNERDEAD`), it means the caller has been granted ownership of the mutex, but the previous owner terminated abnormally.
-   The database is now in a potentially inconsistent state. The `mdb_mutex_failed` function's job is to perform recovery.
-   Its primary action is to call [`mdb_reader_check0()`](Lib/mdb_lock.cpp:213) to clean up any stale reader locks left by the dead process (or any other dead processes).
...
-   If the dead thread belonged to the *current* process, the environment is considered irrecoverably corrupted (`MDB_FATAL_ERROR`), and a `MDB_PANIC` error is returned.
...
-   The `mdb_reader_pid` function is a key abstraction. Its contract is simple: return 0 if a process is certainly dead, and non-zero otherwise. The underlying implementation can be complex, but the interface is clean.
-   Understanding the `MDB_OWNERDEAD` recovery path is crucial for debugging rare and hard-to-reproduce deadlocks or corruption issues that can arise from process crashes.