...
The main function for this is [`mdb_reader_check0()`](Lib/lock.cpp:213). It is called to find and purge reader table entries left behind by dead processes.
...
2.  **Check Liveness**: It then iterates through this list of unique PIDs. For each PID, it calls [`mdb_reader_pid()`](Lib/lock.cpp:23) to determine if the process is still alive.
3.  **Clear Stale Entries**: If [`mdb_reader_pid()`](Lib/lock.cpp:23) reports that a process is dead, the function must clear its entries from the reader table.
...
-   Its primary action is to call [`mdb_reader_check0()`](Lib/lock.cpp:213) to clean up any stale reader locks left by the dead process (or any other dead processes).
